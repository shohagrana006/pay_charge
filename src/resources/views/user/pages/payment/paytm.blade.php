...
<script type="application/javascript" crossorigin="anonymous"
    src="{{$paymentMethod->payment_parameter->PAYTM_ENVIRONMENT}}/merchantpgpui/checkoutjs/merchants/{{$paymentMethod->payment_parameter->PAYTM_MID}}.js">
</script>
@endpush
            ...
            <div class="col-xl-12">
                <div class="card">
                    ...
                    <div class="card-body text-center">
                        <h3>{{shortAmount($paymentLog->final_amount)}} {{$paymentLog->paymentGateway->currency->name}}
                        </h3>
                        <form id="paymentForm">
                            @csrf

                            <div class="form-submit">
                                <button type="submit" class="mt-3 btn btn--primary text-light payment-btn"
                                    id="JsCheckoutPayment">@lang('Pay With Paytm')</button>
                            </div>
                        </form>
                    </div>
                </div>
        ...
        <div class="payment-getway-container">
            <div class="payment-getway">
                <div class="payment-getway-content">
                    <h4>@lang('Payment with Paytm')</h4>
                    <h3>{{shortAmount($paymentLog->final_amount)}}
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.success) {
                ...
                openJsCheckoutPopup(response.orderId, response.txnToken, response.amount);
            } else {
