                            ...
                            <div class="card-header bg--lite--violet">
                                <div class="row">
                                    <div class="col-lg-10 col-sm-10 col-md-6 mt-3">
                                        <span class="card-title text-dark">{{ decode('Reply to customer') }}</span>
                                    </div>
                                    @if($ticket->status != 4)
                                ...
                                @foreach($ticket->messages as $meg)
                                    @if($meg->admin_id == null)
                                        <div class="row shadow-lg p-2 mb-3 bg-light rounded">
                                            <div class="col-lg-3 text-end">
                                                <p>{{ decode('Created at')}} {{($meg->created_at)->format("d-m-Y h:i:s")}}</p>
                                                <h5>{{$ticket->name}}</h5>
                                            </div>
                                                ...
                                                @if($meg->supportfiles()->count() > 0)
                                                    <div class="my-3">
                                                        @foreach($meg->supportfiles as $key=> $file)
                                                            <a href="{{route('admin.support.ticket.file.download',encrypt($file->id))}}" class="mr-3 text-dark"><i class="fa fa-file"></i>{{ decode('File')}} {{++$key}}</a>
                                                        @endforeach
                                                    </div>
                                                @endif
                                            </div>
                                    ...
                                    @else
                                        <div class="row shadow-lg p-2 mb-3 bg-dark rounded">
                                            <div class="col-lg-3 text-end">
                                                <p class="text--light">{{ decode('Created at')}} {{ ($meg->created_at)->format("d-m-Y h:i:s") }}</p>
                                            ...
                                            <div class="col-lg-9">
                                                ...
                                                @if($meg->supportfiles()->count() > 0)
                                                    <div class="my-3">
                                                        @foreach($meg->supportfiles as $key=> $file)
                                                            <a href="{{route('admin.support.ticket.file.download',encrypt($file->id))}}" class="mr-3 text--light"><i class="fa fa-file"></i>  {{ decode('File')}} {{++$key}} </a>
                                                        @endforeach
                                                    </div>
                                                @endif
                                            </div>
