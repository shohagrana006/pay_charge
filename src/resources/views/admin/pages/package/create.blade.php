    ...
    <section>
        ...
        <div class="white-auto-fill">
            <div class="section-container rounded py-3">
                <h5>{{ decode('Package Create') }}</h5>
            </div>
            <form action="{{route('admin.package.store')}}" method="POST" enctype="multipart/form-data" class="row g-3 needs-validation label_margin_top">
                @csrf
                                ...
                                <div class="row">
                                    <div class="col-md-12 ">
                                        ...
                                        <div class="input-container position-relative ps-5">
                                            ...
                                            <input placeholder="Enter Package Name" type="text" name="name[{{$language->code}}]" value="{{ old('name.'.$language->code) }}" class="form-control  border-0 rounded-0" id="validationCustom01">
                                        </div>
                                        @if(session()->get('locale') == $language->code)
                            ...
                            <div class="row">
                                <div class="col-md-12 mt-3">
                                    ...
                                    <div class="input-container position-relative ps-5">
                                        ...
                                        <select name="service_id[]" class="form-control  @error('service_id') is-invalid @enderror border-0 rounded-0 js-example-basic-multiple" data-trigger-name="choices-single-groups" id="choices-single-groups" multiple="multiple">
                                            <option value=''>{{decode('Select Service name')}}</option>
                                            @foreach($services as $service)
                                                <option value="{{$service->id }}" alt="">{{$service->name }} </option>
                                            @endforeach
                                        </select>
                                     </div>
                                </div>
