...
class FlutterwavePaymentController extends Controller
{
    ...
    public function callback($track, $type){
        ...
        if ($type == 'error') {
            ...
            if (isset($track)) {
                $paymentMethod = PaymentMethod::where('unique_code','FLUTTERWAVE105')->first();
                if(!$paymentMethod){
                ...
                if ($response->data->status == "successful" && $response->data->chargecode == "00" && $amount == $response->data->amount && $paymentMethod->currency->name == $response->data->currency && $paymentMethod->status == '1') {
